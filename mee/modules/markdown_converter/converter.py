...
def page_sort_key(filename: str):
    """从 ``{stem}_{N}.md`` 提取页码 N 用于排序。

    优先取文件名尾部（扩展名前）的 ``_数字``；旧格式 ``page_N`` 也兼容。
    取不到返回 None，交由调用方回落自然序。
    """
...
def merge_markdown_files(folder_path: str) -> Optional[str]:
    """合并文件夹内所有 markdown（跳过 *_merged.md），按页码升序拼接。"""
    ...
    keyed = [(page_sort_key(name), name) for name in md_files]
    if any(k is None for k, _ in keyed):
        logger.warning("目录 %s 存在无法解析页码的 md 文件，按名称自然排序", folder_path)
        md_files = sorted(md_files)
    else:
        md_files = [name for _, name in sorted(keyed, key=lambda x: x[0])]
